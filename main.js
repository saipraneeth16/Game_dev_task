...
function create() {
  ...
  this.justDivideHeader = this.add.text(
    this.scale.width / 2,
    70,
    'JUST DIVIDE',
    {
  ...
  this.add.image(720, 220, 'cat').setScale(0.9).setDepth(8);

  this.add.image(560, 300, 'levelBadge').setDepth(6);
  ...
  this.add.image(880, 300, 'scoreBadge').setDepth(6);
  this.score = 0;
  ...
  for (let r = 0; r < 4; r++) {
    for (let c = 0; c < 4; c++) {
      ...
      const slot = this.add.image(x, y, 'gridSlot').setDepth(2);
      slot.setOrigin(0);
...
function spawnActiveFromQueue(scene) {
  ...
  const tile = scene.add.image(x, y, getTileKeyForValue(value))
...
function getTileKeyForValue(value) {

  if (value === 2) return 'tileBlue';
  if (value === 3) return 'tileGreen';
  if (value === 4) return 'tilePurple';
